...
history = th.train(model=model, train_loader=train_loader, test_loader=test_loader, optimizer=optimizer, loss_fun=loss_fun, epochs=epochs, device=device, checkpoint_dir=f"{checkpoint_dir}/{model_type}/{checkpoint_file}", encoding=encoding, model_type=model_type, dataset = "CIFAR10DVS", history=history, debug=debug)
